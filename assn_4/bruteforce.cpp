...
int main()
{
    ...
    read_file.open("key_out.txt");
    possibleKeys.open("finalKey.txt",ios::trunc);

    getline(read_file,half_key);
    BYTE plain[9]={0,0,0,0,0,0,0,0,'\0'};
    // no need to put value
    BYTE cipher1[9]={108,173,173,166,59,116,140,22,'\0'};
    string cipherHex="jpnqnqnjgokhlpej";
    char baseStart='d';
    for(int i=0;i<cipherHex.length();i+=2)
    {
        int value=cipherHex[i]-baseStart;
        value*=16;
        value+=(cipherHex[i+1]-baseStart);
        cipher1[i/2]=value;
    }
    cout<<endl;
    int cnt=0;
    ...
    for(int i = 0; i < (1<<cnt); ++i){
        ...
        for(int j = 0; j < 56; j++) key[j]=test[j]-'0';
        for(int j = 0; j < 56; j++) key2[j]=key[j];
        ...
        h=0;
        for(int j = 0; j < 8 && h == 0; j++){
            if(int(cipher1[j]) != int(cipher2[j]))  h=1;
        }
