...
int main()
{
    ...
    string pswdOut="fdprrirpgfdpffkhnepkppohjhjhhjim";
    cout<<pswdOut<<endl;
    char baseStart='d';
    BYTE plain1[9];
    for(int i=0;i<8;i++)
    {
        int hex=pswdOut[i*2]-baseStart;
        hex=hex*16+pswdOut[i*2+1]-baseStart;
        plain1[i]=hex;
        cout<<hex<<" ";
    }
    cout<<endl;
    ...
    for(int i=0;i<8;i++)
    {
        int hex=pswdOut[16+i*2]-baseStart;
        hex=hex*16+pswdOut[16+i*2+1]-baseStart;
        plain2[i]=hex;
        cout<<hex<<" ";
    }
    cout<<endl;
    ...
    ifstream possibleKeys;
    ofstream possiblePassword;
    possibleKeys.open("finalKey.txt");
    possiblePassword.open("finalPassword.txt",ios::trunc);
    string finalKey;
    getline(possibleKeys,finalKey);
    cout<<finalKey<<endl;
    for(int i=0;i<56;i++)
        key[i]=finalKey[i]-'0';
    ...
    des(plain1,cipher2,r,flag);
    for(int i = 0; i < 8; i++)
    {
        cout << int(cipher2[i])<<' ';
        possiblePassword<<char(cipher2[i]);
    }

    des(plain2,cipher2,r,flag);
    for(int i = 0; i < 8; i++)
    {
        cout << int(cipher2[i])<<" ";
        possiblePassword<<(char)(cipher2[i]);
    }
    possiblePassword<<endl;
    possiblePassword.close();
    return 0;
}
